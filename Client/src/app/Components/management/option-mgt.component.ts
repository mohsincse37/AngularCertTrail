                    ...
                    <tr>
                      ...
                      <td>
                        @if (opt.optionImgPath) {
                          <img [src]="'http://localhost:5241' + opt.optionImgPath" class="rounded border shadow-sm" style="width: 40px; height: 40px; object-fit: cover;">
                        } @else {
...
export class OptionMgtComponent implements OnInit {
    ...
    openModal(opt?: any) {
        if (opt) {
            ...
            this.previewUrl.set(opt.optionImgPath ? 'http://localhost:5241' + opt.optionImgPath : null);
        } else {
